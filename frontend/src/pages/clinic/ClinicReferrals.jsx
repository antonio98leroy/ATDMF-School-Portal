...
export default function ClinicReferrals() {
  ...
  useEffect(() => {
    clinicApi.getReferrals({ page_size: 500 })
      .then((response) => {
  ...
  return (
    <Box>
      ...
      <Typography color="text.secondary" mb={3}>
        Review clinic, health-center and hospital referrals.
      </Typography>
            ...
            {visibleRecords.map((item) => (
              <Paper key={item.id} variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                <Typography fontWeight={900} color="#071B54">
                  {item.patient_name || `Referral ${item.id}`}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  {item.facility_name} · {item.urgent ? "Urgent" : "Routine"} · {item.referred_at}
                </Typography>
              </Paper>
            ))}
