...
export default function ClinicMedicines() {
  ...
  useEffect(() => {
    clinicApi.getMedicines({ page_size: 500 })
      .then((response) => {
  ...
  return (
    <Box>
      ...
      <Typography color="text.secondary" mb={3}>
        Review medicine stock, strengths, dosage forms, batches and expiry dates.
      </Typography>
            ...
            {visibleRecords.map((item) => (
              <Paper key={item.id} variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
                ...
                <Typography variant="body2" color="text.secondary">
                  Stock: {item.quantity_in_stock} · Reorder level: {item.reorder_level} · {item.low_stock ? "LOW STOCK" : "Available"}
                </Typography>
              </Paper>
            ))}
